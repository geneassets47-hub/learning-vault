...
STAT_DEFS = {
    ...
    "kenko_sleep":    "健康／睡眠：睡眠の質・回復・サーカディアンリズム",
    "kenko_food":     "健康／食事：栄養・食習慣・サプリメント・腸内環境",
    "kenko_exercise": "健康／運動：筋トレ・有酸素・柔軟性・身体パフォーマンス",
    "kenko_mental":   "健康／メンタル：集中力・ストレス管理・マインドセット・習慣化",
}
...
STAT_KEYS = {
    ...
    "kenko_sleep":    ["睡眠","回復","疲労","サーカディアン","休息","ぐっすり"],
    "kenko_food":     ["食事","栄養","サプリ","腸内","食習慣","プロテイン","糖質","断食"],
    "kenko_exercise": ["筋トレ","運動","トレーニング","有酸素","筋肉","ジム","ストレッチ"],
    "kenko_mental":   ["メンタル","集中","ストレス","マインド","習慣化","モチベ","瞑想"],
}
...
- その主張が「なぜ正しいと言えるのか」を、発言者が挙げたデータ・事例・論理で裏付ける。根拠なき結論は禁止。
- 「誰が」「何を」「なぜ」を必ず明示する。発言者や登場人物がいれば名前・肩書・立場を書く。
- 議論の流れ・因果関係を再現する。「Aだから→B、しかしCという反論もあり→結局Dが結論」のように論理の筋道を追える形で書く。
- 専門用語・固有名詞・数字は省略せず、そのうえで必ず噛み砕いて説明する。
- 抽象的なきれいごとは禁止。読んだ人が論理を再構成できる具体性で書く。
...
{{
  ...
  "summary": "以下の構造で書く。①【主張】この動画/記事が一番言いたいことを1文で。②【根拠】その主張を支えるデータ・数字・事例を箇条書きで列挙（例：『サムスンの営業利益率72%→メモリ価格高騰が原因』）。③【論理展開】発言者がどういう順番でその結論に至ったか、因果関係の流れを再現（A→B→だからC）。④【対立意見・留保】反論や条件付きがあれば明記。登場人物が複数なら各自の立場・主張を対比。見出し記号（【】）はsummary内では使わず、番号①②③④で区切る。",
  "insight": "あっきーが具体的に何をすればいいか。①行動（『次に物件を検討する時に〇〇を確認する』レベルの具体性）、②判断基準（『△△の判断では□□を基準にする』）、③理由（なぜそうすべきか）の3点セットで書く。『意識すべき』のような精神論は禁止。",
